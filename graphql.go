...
func (s *GraphQLService) PublicURL(projectID, subgraphName, versionOrTag string) string {
	return fmt.Sprintf("%s/public/%s/subgraphs/%s/%s/gn", s.baseURL, projectID, subgraphName, versionOrTag)
}
...
func (s *GraphQLService) PrivateURL(projectID, subgraphName, versionOrTag string) string {
	return fmt.Sprintf("%s/private/%s/subgraphs/%s/%s/gn", s.baseURL, projectID, subgraphName, versionOrTag)
}
